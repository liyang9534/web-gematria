...
import type { CreateOrderResult, OrderInsertData } from "@/lib/payments/types";
...
export async function createOrderWithIdempotency(
  ...
  // Idempotency check
  const existingOrder = await getDb()
  ...
  // Create new order
  const [insertedOrder] = await getDb()
    .insert(ordersSchema)
    .values(orderData)
...
export async function findOriginalOrderForRefund(
  ...
  const originalOrderResults = await getDb()
    .select()
    .from(ordersSchema)
    .where(
      and(
...
export async function updateOrderStatusAfterRefund(
  ...
  const isFullRefund =
    Math.abs(Math.abs(refundedAmount) - originalAmount) <=
    REFUND_TOLERANCE_CENTS;
