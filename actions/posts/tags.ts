...
export async function createTagAction({
  ...
  try {
    const existingTag = await getDb()
      ...
      .from(tagsSchema)
      .where(and(eq(tagsSchema.name, name), eq(tagsSchema.postType, postType)))
      .limit(1);
...
export async function deleteTagAction({
  ...
  try {
    await getDb().delete(tagsSchema).where(eq(tagsSchema.id, id));
